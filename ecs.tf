...
data "aws_iam_policy_document" "ingestion_ecs_cluster" {
  ...
  statement {
    ...
    resources = data.terraform_remote_state.common.outputs.ami_ecs_test_services ? [aws_cloudwatch_log_group.ingestion_ecs_cluster.arn, data.terraform_remote_state.common.outputs.ami_ecs_test_log_group_arn] : [aws_cloudwatch_log_group.ingestion_ecs_cluster.arn]
  }
